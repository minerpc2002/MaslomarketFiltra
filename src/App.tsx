...
export default function App() {
  ...
  const performSearch = async (ai: GoogleGenAI, query: string, modelIndex: number): Promise<SearchResult> => {
    ...
    const response = await ai.models.generateContent({
      ...
      1. ПОИСК OEM: Сначала найди точный оригинальный OEM номер детали.
      2. ГЛАВНЫЙ ПРИОРИТЕТ (КРОСС-НОМЕРА): Пробивай найденные OEM номера строго по официальным каталогам JS ASAKASHI и VIC. Для японских автомобилей (JDM) JS Asakashi и VIC являются главными источниками истины.
      3. ДОПОЛНИТЕЛЬНЫЕ АНАЛОГИ: Только после проверки по JS Asakashi и VIC, ищи кросс-номера в каталогах MANN-FILTER и FILTRON.
      4. ВЕРИФИКАЦИЯ: ОБЯЗАТЕЛЬНО сверяй кросс-номера с OEM. ЗАПРЕЩЕНО угадывать номера, выдавать "примерно подходящие" или выдумывать артикулы. Если нет точного кросса - возвращай null.
      
      ПРАВИЛА ВЫВОДА: 
      1. Обязательно укажи OEM номер.
      2. Если для одной компании есть несколько подходящих аналогов, перечисли их ВСЕ в массиве.
      3. Выдавай данные ТОЛЬКО если найден хотя бы один аналог. Если аналогов нет, верни null для этого типа фильтра.
      4. Внимание к JDM: для правого руля (Daihatsu, Toyota и др.) артикулы JS Asakashi (например AC933, A752) и VIC имеют наивысший приоритет.`,
      config: {
